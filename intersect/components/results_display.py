...
from config.constants import TABLE_SIZE, DISPLAY_COLUMNS
...
def process_search(df: pd.DataFrame, input_text: str) -> None:
    ...
    with st.spinner():
        df = process_semantic_search(openai_client, df, input_embedding)
...
def process_semantic_search(client: OpenAI, df: pd.DataFrame, input_embedding: list) -> pd.DataFrame:
...
def display_df(df: pd.DataFrame, display_cols: dict) -> None:
