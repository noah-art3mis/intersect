...
def render_search_form():
    ...
    with st.form("my_form", border=False):
        ...
        # Job search form
        keywords = st.text_input("Job keywords", placeholder=DEFAULT_KEYWORDS, 
        ...
        with st.expander("Advanced search options"):
            ...
            with col2:
                location = st.text_input("City (UK)", placeholder="london", 
                                       help="Enter city name")
                contract_type = st.selectbox("Contract type", ["Any", "permanent", "contract", "temp"], 
                                           help="Type of employment")
                full_time = st.checkbox("Full time only", value=True, 
                                      help="Show only full-time positions")
